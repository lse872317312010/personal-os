...
| M2-G2 | Flutter Android shell | analyze、widget tests、APK build | Redmi Turbo 安装、冷启动、离线闭环 | secure composition、observation recording/history UI、controlled Photo Picker 与 Camera wiring 已存在；Camera 拍摄时显式请求权限，并通过 app-private cache/FileProvider、opaque token、native blob sink 及 cleanup/rollback 边界接入；当前工作区未提供 Flutter SDK，未编译，Android 运行时与真机证据未取得 |
